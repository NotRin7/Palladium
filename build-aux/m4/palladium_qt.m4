dnl Copyright (c) 2013-2016 The Palladium Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set palladium_enable_qt to false. Else, exit.
AC_DEFUN([PALLADIUM_QT_FAIL],[
  if test "x$palladium_qt_want_version" = xauto && test "x$palladium_qt_force" != xyes; then
    if test "x$palladium_enable_qt" != xno; then
      AC_MSG_WARN([$1; palladium-qt frontend will not be built])
    fi
    palladium_enable_qt=no
    palladium_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([PALLADIUM_QT_CHECK],[
  if test "x$palladium_enable_qt" != xno && test "x$palladium_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([PALLADIUM_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build palladium-qt GUI (default=auto)])],
    [
     palladium_qt_want_version=$withval
     if test "x$palladium_qt_want_version" = xyes; then
       palladium_qt_force=yes
       palladium_qt_want_version=auto
     fi
    ],
    [palladium_qt_want_version=auto])
...
dnl Outputs: palladium_enable_qt, palladium_enable_qt_dbus, palladium_enable_qt_test
AC_DEFUN([PALLADIUM_QT_CONFIGURE],[
  ...
  if test "x$palladium_cv_static_qt" = xyes; then
  ...
  PALLADIUM_QT_CHECK([
    palladium_enable_qt=yes
    palladium_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      palladium_enable_qt_test=no
    fi
    palladium_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      palladium_enable_qt_dbus=yes
    fi
  ...
  if test x$palladium_enable_qt = xyes; then
    AC_MSG_RESULT([$palladium_enable_qt ($QT_LIB_PREFIX)])
  else
    AC_MSG_RESULT([$palladium_enable_qt])
  fi
...
dnl Output: palladium_cv_qt5=yes|no
AC_DEFUN([_PALLADIUM_QT_CHECK_QT5],[
  AC_CACHE_CHECK(for Qt 5, palladium_cv_qt5,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [palladium_cv_qt5=yes],
    [palladium_cv_qt5=no])
])])

dnl Internal. Check if the included version of Qt is greater than Qt58.
dnl Requires: INCLUDES must be populated as necessary.
dnl Output: palladium_cv_qt58=yes|no
AC_DEFUN([_PALLADIUM_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, palladium_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [palladium_cv_qt58=yes],
    [palladium_cv_qt58=no])
])])
...
dnl Output: palladium_cv_static_qt=yes|no
dnl Output: Defines QT_STATICPLUGIN if plugins are static.
AC_DEFUN([_PALLADIUM_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, palladium_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
      ...
      [palladium_cv_static_qt=yes],
      [palladium_cv_static_qt=no])
    ])
  if test "x$palladium_cv_static_qt" = xyes; then
...
AC_DEFUN([_PALLADIUM_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$palladium_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, palladium_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
           ...
           [palladium_cv_need_platformsupport=yes],
           [palladium_cv_need_platformsupport=no])
         ])
         if test "x$palladium_cv_need_platformsupport" = xyes; then
           if test x$palladium_cv_qt58 = xno; then
...
dnl Inputs: palladium_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If palladium_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: palladium_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_PALLADIUM_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  PALLADIUM_QT_CHECK([
    if test "x$palladium_qt_want_version" = xauto; then

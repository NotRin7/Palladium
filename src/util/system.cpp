...
const char * const PALLADIUM_CONF_FILENAME = "palladium.conf";
 ...
 * https://github.com/palladium/palladium/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //palladium-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "palladium";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Palladium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Palladium
    // Mac: ~/Library/Application Support/Palladium
    // Unix: ~/.palladium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Palladium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Palladium";
#else
    // Unix
    return pathRet / ".palladium";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", PALLADIUM_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Palladium Core copyright is not removed by accident
    if (copyright_devs.find("Palladium Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Palladium Core developers";
    }

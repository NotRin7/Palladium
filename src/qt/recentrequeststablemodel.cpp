    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return PalladiumUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, PalladiumUnits::separatorNever);
            else
                return PalladiumUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+PalladiumUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
